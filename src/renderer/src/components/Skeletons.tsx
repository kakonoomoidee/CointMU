 ...
 */
export function SkeletonList({ itemCount = 5 }: { itemCount?: number }): JSX.Element {
  return (
 ...
 */
export function SkeletonTable({ rowCount = 5, columns = 4 }: { rowCount?: number, columns?: number }): JSX.Element {
  return (
    <div className="w-full">
      ...
      <div className="divide-y divide-slate-100">
        {Array.from({ length: rowCount }).map((_, r) => (
          <div key={`tr-${r}`} className="flex w-full px-5 py-4 hover:bg-slate-50/50 transition-colors">
            {Array.from({ length: columns }).map((_, c) => (
              <div key={`td-${c}`} className="flex-1 flex items-center">
                {c === 0 ? (
                  ...
                  <Skeleton className={`w-20 h-4 ${c === 0 ? '' : 'ml-auto'}`} />
                )}
              </div>
            ))}
          </div>
