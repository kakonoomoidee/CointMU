...
import type { CustomNetwork } from '@/views/Settings'
 ...
 */
export function AddNetworkModal({ isOpen, onClose, onSave }: AddNetworkModalProps): JSX.Element | null {
  ...
  const handleSave = (): void => {
    ...
    if (!name.trim()) {
      setError(t('settings.network.addNetwork.networkNameLabel') + ' is required')
      return
    }
    ...
    try {
      ...
      setError(t('settings.network.addNetwork.rpcUrlLabel') + ' must be a valid HTTP/HTTPS URL')
      return
    }

    const parsedChainId = parseInt(chainId, 10)
    if (isNaN(parsedChainId) || parsedChainId <= 0) {
      setError(t('settings.network.addNetwork.chainIdLabel') + ' must be a positive number')
      return
    }

    if (!symbol.trim()) {
      setError(t('settings.network.addNetwork.currencySymbolLabel') + ' is required')
      return
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="text-lg font-bold text-slate-800">{t('settings.network.addNetwork.modalTitle')}</h3>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">{t('settings.network.addNetwork.networkNameLabel')}</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">{t('settings.network.addNetwork.rpcUrlLabel')}</label>
            <input
              type="text"
              value={rpcUrl}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">{t('settings.network.addNetwork.chainIdLabel')}</label>
            <input
              type="number"
              value={chainId}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">{t('settings.network.addNetwork.currencySymbolLabel')}</label>
            <input
              type="text"
              value={symbol}
