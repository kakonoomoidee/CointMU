  ...
  return (
    <header className="flex items-center justify-between px-8 py-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-slate-800">{t('mining.header.title')}</span>
      </div>

      <div className="flex items-center gap-3">
        {isMining ? (
          <StatusPill tone="success" label={t('mining.header.statusMining')} pulse />
        ) : powerStatus === 'Paused (Battery)' ? (
          <StatusPill tone="warning" label={powerStatus} />
        ) : (
          <StatusPill tone="neutral" label={t('mining.header.statusStopped')} showDot={false} />
        )}
