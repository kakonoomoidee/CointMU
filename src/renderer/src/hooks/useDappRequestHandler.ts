...
export function useDappRequestHandler(): void {
  ...
  useEffect(() => {
    const unsubscribe = window.api.dapp.onDappRequest((payload) => {
      ...
      const isConnected = connectedSites.includes(origin)
      const isConnectionRequest = method === 'eth_requestAccounts'
      const isWriteOperation = method === 'eth_sendTransaction' || method === 'personal_sign'
      const isReadOnlyOperation = ['eth_chainId', 'eth_getBalance', 'eth_accounts', 'net_version', 'eth_blockNumber', 'eth_gasPrice', 'eth_estimateGas'].includes(method)
