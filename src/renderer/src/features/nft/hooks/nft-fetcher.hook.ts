 ...
 */
async function runDiscovery(walletAddress: string, signal: AbortSignal): Promise<NFTMetadata[]> {
  ...
  const existingKeys = new Set(
    existingFiltered.map((nft) => `${nft.contractAddress.toLowerCase()}:${nft.tokenId}`)
  )
  ...
  const settled = await Promise.allSettled(
    genuinelyNew.map((token) =>
      resolveNFTMetadata(token.contractAddress, token.tokenId, token.standard, provider)
    )
  )
  ...
  useEffect(() => {
    ...
    const run = async (silent: boolean): Promise<void> => {
      ...
      try {
        ...
        if (controller.signal.aborted) return
        const message = err instanceof Error ? err.message : 'NFT discovery failed'
